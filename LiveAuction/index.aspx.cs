    ...
    public partial class index : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    //Response.Write("<br/>" + Convert.ToDateTime(dt.Rows[i]["AuctionDate"]));
                    litLiveAuction = litLiveAuction + @"<div class='col-md-3 col-sm-6 col-xs-12'>
                            <div class='action-item-sec text-center'><input type='hidden' class='auctionId' value=" + i +
                                    "><img src='/TCAG/Admin/FileUpload/" + dt.Rows[i]["ImageName"] + "' width='200px' alt='" + dt.Rows[i]["AuctionName"] + " image" + "'/><p>" + dt.Rows[i]["Address"] +
                                            "</p><a href='#' class='btn btn-danger btn-block bidding-sing-btn todaysAction' data-toggle='modal' data-target='#bidpopup" + dt.Rows[i]["AuctionId"] + "' >View Details</a><div class='action-timing-sec'><p id='counterDiva" + i + "'>LIVE";


                    // Response.Write(auctionDate +" ");
                    litLiveAuction = litLiveAuction + @"<script type='text/javascript'>
                            $(function () {
                    ...
                    litLiveAuction = litLiveAuction + @"</p></div></div></div>";
